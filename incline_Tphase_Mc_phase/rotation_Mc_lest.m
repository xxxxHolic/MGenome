...
%% 输入参数
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
filename = 'PMNPT_incline_lest' 
a = 4.00;                         
b = 4.00;                          
c = 4.03;                     
aa = 89.95*pi/180; bb = pi/2;cc = pi/2; 
ss = [1 0 0;...
      cos(cc) sin(cc) 0;...
...
%    if     rotate(1)
       c = vpa(Euler(0,0,0));
       l = vpa(L_0_1*c);
       domain_1(:,:) = q_space(l,length,c);
%    end
%    if rotate(2)
%        c = vpa(Euler(0,0,th));
%        l = vpa(L_0_1*c);
%        domain_2(:,:) = q_space(l,length,c);
% %    end
% %    if rotate(3)
%        c = vpa(Euler(0,0,2*th));
%        l = vpa(L_0_1*c);
%        domain_3(:,:) = q_space(l,length,c);
% %    end
% %    if rotate(4)
%        c = vpa(Euler(0,0,-th));
%        l = vpa(L_0_1*c);
%        domain_4(:,:) = q_space(l,length,c);
%    end
% % %    if rotate(5)
